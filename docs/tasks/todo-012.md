...
- [x] 核对依赖合入，逐项检查关键操作是否已有审计写入；发现缺失时列出所属模块补丁，不默默用日志猜测补数。
- [x] 先写固定数据集的指标计算、时间边界、零分母、非管理员访问和密钥脱敏测试，运行确认目标行为失败。
- [x] 明确定义每项指标的来源、时区和过滤规则，实现只读聚合查询；失败/取消消息不能计作成功回答。
- [x] 实现管理员审计筛选、分页和详情，最小化展示敏感内容；审计记录保留操作者、资源、动作、结果和时间，默认不存完整密码/密钥或无必要聊天原文。
- [x] 实现结构化请求/错误日志关联与脱敏测试，确保 provider 错误也不泄露 Authorization；日志保留策略写入部署说明。
- [x] 用 ECharts 实现趋势与分类统计，空数据有明确状态；前端显示与后端同一指标口径，不自行计算另一套分母。
- [x] 用人工可核对的固定数据逐项对照 API 与 UI 数值，并验证匿名/user/agent 均不能越权查看管理员统计或审计。
- [x] 执行相关测试与静态检查，独立评审查询效率、隐私最小化和统计口径；记录并提交 `in_review`，按 WORKFLOW 合并收尾。
...
- 原始日志、评审与截图保留在忽略的 `.local/check-final.log`、`.local/analytics-full-review.md`、`.local/admin-stats-desktop.png`、`.local/admin-stats-mobile.png`、`.local/admin-trend-mobile.png`。本段验收记录提交时任务为 `in_review`；实际功能合并及状态收尾见下文。
### 2026-09-26 功能合并与任务收尾

- 功能分支 `feat/todo-012-analytics`，代码提交 `e10d08c912d14f47b8878c84641282be41fafa18`，已验证PR头 `c93d379af9df2d16f6aff0d80b498a35e82059bd`。
- 功能 [PR #25](https://github.com/cdzdd/aiServerAndModelTraining/pull/25) 已于2026-09-26 09:03:13 UTC实际普通squash合并，真实功能合并SHA：`7d26b91bebb67a1b7adac562b75cd9667c43ce98`。重新fetch后，其完整Git树与已验证PR头完全相同。
- 合并前重新核对main/PR head/干净工作区与服务器规则：main无branch protection、rulesets为空、无required checks或必要GitHub审批；独立代码评审及本地验收均已通过。未修改保护、使用admin或伪造check。云端CI未运行。
- 此收尾分支 `docs/todo-012-close` 仅修改本任务文档，核对diff格式、Markdown链接、实际功能PR与合并SHA；代码不变，复用上述675后端/114前端/23浏览器与静态/构建/迁移验收。本收尾PR实际合入权威main后，任务为done，无需再建PR记录收尾PR自身。
- 实现范围内无未解决阻塞；费用与生产日志保留参数仍按契约显示未知/由部署负责人配置，不虚构账单或生产规模效果。本地原始验证证据保留于独立worktree。
