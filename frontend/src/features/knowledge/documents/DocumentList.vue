...
async function load(target=page.value) {
  ...
  try {
    ...
    if(items.value.some(item=>['uploaded','processing','parsed'].includes(item.status) && ['queued','running'].includes(item.latest_job?.state ?? '')))
...
async function change(item:DocumentSummary,action:'toggle'|'delete'|'reindex') {
  ...
  try {
    ...
    if(current===generation) {message.value=action==='delete'?'文档已删除。':action==='reindex'?'已提交处理任务。':'文档状态已更新。';await load(page.value)}
  } catch(cause) {if(current===generation)error.value=errorMessage(cause)}
...
<template>
  <section class="document-list" aria-label="文档">
    ...
    <article v-for="item in items" :key="item.id" class="panel document-card">
      ...
      <p v-if="item.active_revision && ((item.candidate_revision_id && item.candidate_revision_id!==item.active_revision_id) || ['uploaded','processing','parsed','failed'].includes(item.status))">新版本处理：{{ statusLabel(item.status) }}<span v-if="item.candidate_revision"> · {{ item.candidate_revision.filename }}</span></p>
      ...
      <div class="actions">
        ...
        <template v-if="admin"><ElButton :disabled="saving" @click="uploadFor=item.id">上传新版本</ElButton><ElButton :disabled="saving" @click="change(item,'toggle')">{{ item.status==='disabled'?'启用文档':'停用文档' }}</ElButton><ElButton v-if="item.status==='failed'" :aria-label="item.latest_job?.kind==='index'?'重试索引':'重试解析'" :disabled="saving" @click="change(item,'reindex')">{{ item.latest_job?.kind==='index'?'重试索引':'重试解析' }}</ElButton><ElButton :disabled="saving" @click="change(item,'delete')">删除文档</ElButton></template>
      </div>
    </article>
