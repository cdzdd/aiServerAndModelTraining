...
- 用户已指定开始 todo-001；后续任务仍需用户明确接手指令，不自动把全部规划纳入当前开发范围。
- 2026-09-22 用户明确选择暂时采用“本地验收 + GitHub PR”：按 WORKFLOW 第 10.1 节执行本地检查、评审和普通 PR 合并，不再等待因账号账单限制无法启动的云端 CI。此规则适用于后续接手对话，直到用户同意恢复云端 CI。
...
7. 只有按当前验收模式验证通过并实际合入权威 main，才走状态收尾；等待本地验收、必要审批或合并时保持 `in_review`，不能写成 `done`。云端 CI 未运行不得伪写为通过；服务端强制规则仍须满足。
...
技术契约在 `docs/CONTRACTS.md`。本地安装、启动和统一验证入口见 `scripts/README.md`；后续任务增加测试时应接入统一检查和 CI。仅按真实运行结果报告成功，不把规划命令说成已通过的验证。

当前代码 PR 在本 worktree 运行 `node scripts/dev.mjs check` 及任务专项检查，记录实际命令、环境、结果和对应提交。纯文档 PR 做格式、链接与事实核对。保留 GitHub PR、独立评审与状态收尾；禁止伪造成功 check、使用 `--admin` 或削弱现有分支保护。
